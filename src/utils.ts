...
export const calculateFreeTrailersForDateTime = (
  ...
  if (store.items) {
    const freeTrailers = store.items
      .map((item) => {
        ...
        const hourSlotsWithFreeCapacity = availableHourSlots
          .map((hourSlot) => {
            ...
            return {
              hour: hourSlot.hour,
              freeCapacityUnits: freeSlotCapacityUnits,
            };
          })
        ...
        return {
          id: item.id,
          hourSlotsWithFreeCapacity: hourSlotsWithFreeCapacity,
        };
      })
      .flat()
      .filter((trailer) => trailer.hourSlotsWithFreeCapacity.length > 0);
    return freeTrailers;
  } else return [];
};

export const formatPuuiloUrlString = (inputString: string) =>
  inputString
    .replace(/[äÄ]/g, "a")
    .replace(/[öÖ]/g, "o")
    .replace(",", "")
